...
art/                  art briefs and eventual production assets
...
| `npm run check` | Run validation, tests, and a production build |

## Publishing

Every push to `main` runs the validation suite and deploys `_site/` to GitHub Pages. Pull requests run the same checks without deploying. In repository settings, Pages should use **GitHub Actions** as its source.
